...
export function TransferCriteria({
  ...
  return <div className="space-y-4">
      <div className="flex justify-between items-center">
        ...
        <Button type="button" variant="outline" size="sm" onClick={handleAddCriterion} className="gap-1">
          <Plus className="h-3.5 w-3.5" />
          Add Criterion
        </Button>
      </div>
      
      {criteria.length === 0 ? <div className="text-center py-4 text-sm text-muted-foreground">
          ...
          {criteria.map(criterion => <div key={criterion.id} className="grid grid-cols-12 gap-2 items-center">
              ...
              <div className="col-span-3">
                <Select value={criterion.destinationLotId} onValueChange={value => handleCriterionChange(criterion.id, 'destinationLotId', value)}>
                  <SelectTrigger>
                    <SelectValue placeholder="Destination lot" />
                  </SelectTrigger>
                  <SelectContent>
                    {availableLots.map(lot => <SelectItem key={lot.id} value={lot.id}>
                        {lot.name}
                      </SelectItem>)}
                  </SelectContent>
                </Select>
              ...
              <div className="col-span-1">
                <Button type="button" variant="ghost" size="icon" onClick={() => handleRemoveCriterion(criterion.id)}>
